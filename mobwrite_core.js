...
mobwrite.shareObj.prototype.syncText = function() {
  ...
  if (this.deltaOk) {
    ...
    if (changed) {
      ...
      if (mobwrite.debug) {
            window.console.info('Shadow Text updated to (syncText) ' + this.shadowText);
        }  
    }
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (name == 'F' || name == 'f') {
      ...
      if (file) {
        ...
        if (mobwrite.debug) {
            window.console.info('Shadow Text updated to (decodeURI) ' + file.shadowText);
        }  
        ...
        if (clientVersion != file.clientVersion) {
          ...
          if (diffs && (diffs.length != 1 || diffs[0][0] != DIFF_EQUAL)) {
            ...
            if (name == 'D') {
              ...
              if (mobwrite.debug) {
                window.console.info('Shadow Text updated to (D) ' + file.shadowText);
              }  
              file.setClientText(file.shadowText);
            } else {
              // Merge text.
              ...
              if (mobwrite.debug) {
                window.console.info('Shadow Text updated to ' + file.shadowText);
              }  
